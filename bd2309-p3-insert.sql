...
ALTER TABLE MUSICO 
    DISABLE CONSTRAINT musico_fk_artista;
...
ALTER TABLE MUSICO 
    ENABLE CONSTRAINT musico_fk_artista;
...
-- CANCION --
-- Se asigna el valor 0 al calculado "cuantas_listas". 
-- Se debe calcular al final del script (ver enunciado). 
-- 7 canciones por album, aunque contenga mas en la realidad.
...
UPDATE CANCION C SET cuantas_listas = ( SELECT COUNT(*)
                                        FROM LISTA_CANCION L
                                        WHERE L.album = C.album